...
A single Claude Code Agent Skill (`skills/neurodiversity/`) plus reference
...
1. Edit `skills/neurodiversity/SKILL.md` or the relevant file under
...
4. Re-run the relevant benchmark fixtures (`bench/`) before and after —
   ...
   glance.
5. Sync `~/.claude/skills/neurodiversity/` from `skills/neurodiversity/` if you're
...
6. If you rename, move, or delete a `references/`/`frames/`/`schemas/`
   file, run `node skills/neurodiversity/check-links.js` — it checks every
...
ADHD / NEURODIVERSITY / ADHD→NEURODIVERSITY), the planted defect ID from the
